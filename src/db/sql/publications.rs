...
impl PublicationOperations for SqlClient {
    async fn create_publication(
        ...
        sqlx::query_as::<_, Publication>(
            r#"
            INSERT INTO publications (user_id, title, about, tags, s3key, price, status)
            VALUES ($1, $2, $3, $4, $5, $6, 'PENDING_ONCHAIN')
    ...
    async fn get_publication(&self, publication_id: Uuid) -> Result<Publication, sqlx::Error> {
        sqlx::query_as::<_, Publication>(
            r#"
            SELECT id, user_id, title, about, tags, s3key, price, status, transaction_hash, created_at, updated_at
            FROM publications 
            WHERE id = $1
            "#,
        )
    ...
    async fn get_cited_by(&self, publication_id: Uuid) -> Result<Vec<Publication>, sqlx::Error> {
        sqlx::query_as::<_, Publication>(
            r#"
            SELECT p.id, p.user_id, p.title, p.about, p.tags, p.s3key, p.price, p.status, p.transaction_hash, p.created_at, p.updated_at
            FROM publications p
