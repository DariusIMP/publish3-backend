...
pub trait PurchaseOperations {
    ...
    async fn list_purchases_by_publication(
        &self,
        publication_id: Uuid,
        page: Option<i64>,
        limit: Option<i64>,
...
impl PurchaseOperations for SqlClient {
    async fn create_purchase(&self, new_purchase: &NewPurchase) -> Result<Purchase, sqlx::Error> {
        sqlx::query_as::<_, Purchase>(
            r#"
            INSERT INTO purchases (user_id, publication_id, status, transaction_hash)
            VALUES ($1, $2, $3, $4)
    ...
    async fn get_purchase(&self, purchase_id: Uuid) -> Result<Purchase, sqlx::Error> {
        sqlx::query_as::<_, Purchase>(
            r#"
            SELECT id, user_id, publication_id, status, transaction_hash, created_at, updated_at
            FROM purchases
            WHERE id = $1
            "#,
        )
