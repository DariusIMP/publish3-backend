...
use crate::db::sql::{models::PublicationAuthor, SqlClient};
...
pub trait PublicationAuthorOperations {
    ...
    async fn count_authors_for_publication(&self, publication_id: Uuid) -> Result<i64, sqlx::Error>;
...
impl PublicationAuthorOperations for SqlClient {
    ...
    async fn remove_author_from_publication(
        ...
        sqlx::query(
            "DELETE FROM publication_authors WHERE publication_id = $1 AND author_id = $2",
        )
        .bind(publication_id)
        .bind(author_id)
        .execute(&self.db)
    ...
    async fn count_publications_for_author(&self, author_id: Uuid) -> Result<i64, sqlx::Error> {
        sqlx::query_scalar(
            "SELECT COUNT(*) FROM publication_authors WHERE author_id = $1",
        )
        .bind(author_id)
        .fetch_one(&self.db)
        .await
